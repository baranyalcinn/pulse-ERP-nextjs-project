...
import { BSMGRPLEGEN003 } from '@/services/general/types';
...
export default function CountriesPage() {
  const [data, setData] = React.useState<BSMGRPLEGEN003[]>([]);
  const [filteredData, setFilteredData] = useState<BSMGRPLEGEN003[]>([]);
  ...
  // Düzenleme
  function handleEdit(item: BSMGRPLEGEN003) {
    setEditingId(item.countrycode);
    setValue('comcode', item.comcode);
    setValue('countrycode', item.countrycode);
    setValue('countrytext', item.countrytext);
  }

  // Silme
  async function handleDelete(item: BSMGRPLEGEN003) {
    ...
    try {
      ...
      const { error } = await supabase
        .from('bsmgrplegen003')
        .delete()
        .eq('countrycode', item.countrycode);
      if (error) throw error;
      ...
      <div className="mb-6 flex items-center space-x-4">
        <div className="flex-1">
          <SearchBar
            ...
            placeholder="Ülke kodu veya adı ile ara..."
            className="w-full"
          />
        </div>
        <Button
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-gray-800/50 backdrop-blur-sm p-6 rounded-xl mb-6 border border-gray-700/50">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <Input
              label="Ülke Adı"
              {...register('countrytext', { required: 'Ülke adı zorunlu' })}
              disabled={isLoading}
            />
            {errors.countrytext && (
              <span className="text-red-500 text-sm">{errors.countrytext.message}</span>
            )}
          </div>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-700/50">
                  ...
                  <th className="p-4 text-white font-semibold">Ülke Adı</th>
              ...
              <tbody>
                {filteredData.map((item) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-white">{item.countrytext}</td>
                    <td className="p-4">
